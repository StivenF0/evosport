...
const req = (p: string, o?: RequestInit) => app.handle(new Request(`http://localhost/venue${p}`, o));

describe('VenueRoutes', () => {
  afterEach(() => mock.restore());

  const validVenue = { id: 1, name: 'Maracanã', city: 'Rio', capacity: 78000, latitude: null, longitude: null };

  it('POST /', async () => {
    spyOn(venueService, 'createVenue').mockResolvedValue(validVenue);
    const res = await req('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'Maracanã', city: 'Rio' }) });
  ...
  it('PUT /:id', async () => {
    ...
    const res = await req('/1', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'Maracanã', city: 'Rio' }) });
    expect(res.status).toBe(200);
  });
