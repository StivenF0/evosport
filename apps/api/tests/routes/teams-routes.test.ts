...
describe('TeamRoutes', () => {
  ...
  it('POST /', async () => {
    ...
    const res = await req('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'Brasil' }) });
  ...
  it('PUT /:id', async () => {
    ...
    const res = await req('/1', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'Brasil' }) });
    expect(res.status).toBe(200);
  });
