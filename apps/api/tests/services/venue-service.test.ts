...
describe('VenueService - Update Venue', () => {
  ...
  it('should update a venue.', async () => {
    ...
    const venue = await venueService.updateVenue(1, { capacity: 80000, name: 'Maracanã Atualizado' });
...
describe('VenueService - Get Venues For Map (Business Rule)', () => {
  ...
  it('should filter venues without coordinates and map them to Leaflet format.', async () => {
    spyOn(venueService, 'getAllVenues').mockResolvedValue([
      { id: 1, name: 'Maracanã', city: 'Rio de Janeiro', capacity: 78000, latitude: -22.91, longitude: -43.23 },
      { id: 2, name: 'Estádio Incompleto', city: 'São Paulo', capacity: 40000, latitude: null, longitude: null },
      { id: 3, name: 'Arena Dunnas', city: 'Mossoró', capacity: 45000, latitude: -5.18, longitude: -37.34 }
    ]);
