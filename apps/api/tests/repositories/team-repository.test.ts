...
describe('TeamRepository - Create', () => {
  ...
  it('should create a new team', async () => {
    spyOn(db, 'insert').mockReturnValue({ values: () => ({ returning: () => Promise.resolve([{ id: 1, name: 'Brasil' }]) }) } as any);
...
describe('TeamRepository - Find All', () => {
  ...
  it('should return all teams', async () => {
    spyOn(db, 'select').mockReturnValue({ from: () => Promise.resolve([{ id: 1, name: 'Brasil' }]) } as any);
...
describe('TeamRepository - Update', () => {
  ...
  it('should throw an error if update fails', async () => {
    ...
    expect(teamRepository.update(99, { name: 'BR' })).rejects.toThrow('Time não encontrado para atualização.');
  });
});
