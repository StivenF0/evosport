...
describe('VenueRepository - Create', () => {
  ...
  it('should create a new venue', async () => {
    spyOn(db, 'insert').mockReturnValue({ values: () => ({ returning: () => Promise.resolve([{ id: 1, name: 'Maracanã' }]) }) } as any);
    expect((await venueRepository.create({ name: 'Maracanã', city: 'Rio' }))?.id).toBe(1);
  });

  it('should throw an error if creation fails', async () => {
    spyOn(db, 'insert').mockReturnValue({ values: () => ({ returning: () => Promise.resolve([]) }) } as any);
    expect(venueRepository.create({ name: 'Maracanã', city: 'Rio' })).rejects.toThrow('Falha ao criar a sede.');
  });
});
...
describe('VenueRepository - Find All', () => {
  ...
  it('should return all venues', async () => {
    spyOn(db, 'select').mockReturnValue({ from: () => Promise.resolve([{ id: 1, name: 'Maracanã' }]) } as any);
...
describe('VenueRepository - Find By Id', () => {
  ...
  it('should return a venue by ID', async () => {
    spyOn(db, 'select').mockReturnValue({ from: () => ({ where: () => ({ limit: () => Promise.resolve([{ id: 1 }]) }) }) } as any);
    expect((await venueRepository.findById(1))?.id).toBe(1);
  });

  it('should throw an error if not found', async () => {
    spyOn(db, 'select').mockReturnValue({ from: () => ({ where: () => ({ limit: () => Promise.resolve([]) }) }) } as any);
...
describe('VenueRepository - Update', () => {
  ...
  it('should update a venue', async () => {
    spyOn(db, 'update').mockReturnValue({ set: () => ({ where: () => ({ returning: () => Promise.resolve([{ id: 1 }]) }) }) } as any);
    expect((await venueRepository.update(1, { name: 'Allianz' }))?.id).toBe(1);
  });

  it('should throw an error if update fails', async () => {
    spyOn(db, 'update').mockReturnValue({ set: () => ({ where: () => ({ returning: () => Promise.resolve([]) }) }) } as any);
    expect(venueRepository.update(99, { name: 'Allianz' })).rejects.toThrow('Sede não encontrada para atualização.');
  });
});
...
describe('VenueRepository - Delete', () => {
  ...
  it('should delete a venue', async () => {
    spyOn(db, 'delete').mockReturnValue({ where: () => ({ returning: () => Promise.resolve([{ id: 1 }]) }) } as any);
    expect((await venueRepository.delete(1))?.id).toBe(1);
  });

  it('should throw an error if deletion fails', async () => {
    spyOn(db, 'delete').mockReturnValue({ where: () => ({ returning: () => Promise.resolve([]) }) } as any);
